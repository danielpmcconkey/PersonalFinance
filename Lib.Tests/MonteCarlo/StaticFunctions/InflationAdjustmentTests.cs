...
///
/// Verifies that each spend/tax/payday function correctly scales dollar amounts by
/// cumulativeCpiMultiplier, and that the CumulativeCpiMultiplier field on CurrentPrices
/// compounds correctly each month.
/// </summary>
public class InflationAdjustmentTests
{
    ...
    [Fact(DisplayName = "§16.1 — CalculateMonthlyRequiredSpend multiplies non-debt spend by cumulativeCpiMultiplier")]
    ...
    [Fact(DisplayName = "§16.1b — CalculateMonthlyRequiredSpend with multiplier=1.0 returns same value as baseline (no regression)")]
    public void CalculateMonthlyRequiredSpend_MultiplierOne_MatchesCurrentBehavior()
    {
        // A multiplier of exactly 1.0 must produce the same result as the pre-inflation baseline.
        var person   = CreatePersonBornIn(1980);
        person.RequiredMonthlySpend = 3_000m;
        var model    = TestDataManager.CreateTestModel();
        model.RetirementDate = new LocalDateTime(2050, 1, 1, 0, 0);
    ...
    [Fact(DisplayName = "§16.2 — CalculateMonthlyFunSpend with multiplier=1.10 returns 10% more than multiplier=1.0")]
    ...
    [Fact(DisplayName = "§16.2b — CalculateMonthlyFunSpend with multiplier=1.0 returns same value as no-multiplier baseline")]
    ...
    [Fact(DisplayName = "§16.4 — CalculateCashNeedForNMonths with 1% monthly CPI is greater than with 0% CPI")]
    ...
    [Fact(DisplayName = "§16.4b — CalculateCashNeedForNMonths compounds CPI correctly: month 0 uses multiplier, month 1 uses multiplier×1.01, month 2 uses multiplier×1.01²")]
    ...
    [Fact(DisplayName = "§16.5 — CalculateFunPointsForSpend with multiplier=2.0 earns half the fun points vs multiplier=1.0")]
    ...
    [Fact(DisplayName = "§16.5b — CalculateFunPointsForSpend with multiplier=1.0 matches baseline (no regression)")]
    ...
    [Fact(DisplayName = "§16.6 — CalculateTaxLiabilityForYear with multiplier=2.0 applies double the federal and NC standard deductions")]
    ...
    [Fact(DisplayName = "§16.7 — SS worksheet threshold scales with multiplier: combined income just above base threshold at multiplier=1.0 is below threshold at multiplier=1.5")]
    public void CalculateTaxLiabilityForYear_SsWorksheetThreshold_ScalesWithMultiplier()
    {
        // TaxConstants.SocialSecurityWorksheetCreditLine8 = $32,000.
    ...
    [Fact(DisplayName = "§16.8 — ScheduleD capital loss limit with multiplier=1.5 becomes -$4,500, not -$3,000")]
    public void ScheduleD_CapitalLossLimit_ScalesNegativelyWithMultiplier()
    {
        // TaxConstants.ScheduleDMaximumCapitalLoss = -3000m
    ...
    [Fact(DisplayName = "§16.9 — WithholdTaxesFromPaycheck with multiplier=2.0 applies double the OASDI cap and Medicare surcharge threshold")]
    public void WithholdTaxesFromPaycheck_DoubleMultiplier_ScalesOasdiCapAndMedicareSurcharge()
    {
        ...
        // For a $300k salary at multiplier=1.0: OASDI is capped at $11,439 (actual = 6.2%×$300k = $18,600 > cap).
        // At multiplier=2.0: OASDI cap doubles to $22,878 → now uncapped → OASDI = $18,600/year (+$7,161).
        // Medicare surcharge: at 1.0x threshold=$250k → surcharge on $50k = $450/year.
        //                     at 2.0x threshold=$500k → no surcharge ($300k < $500k) → saves $450/year.
        // Net: OASDI increase ($7,161) >> Medicare savings ($450) → withholding INCREASES at 2.0x.
        Assert.True(atTwo.amount > atOne.amount,
            "For a $300k salary, OASDI uncapping at 2× multiplier (+$7,161/yr) outweighs Medicare threshold savings ($450/yr) → higher total withholding");
    }
    ...
    [Fact(DisplayName = "§16.10 — Tax bracket thresholds scale with multiplier: income at old 22% boundary stays in 12% bracket at higher multiplier")]
    ...
    [Fact(DisplayName = "§16.10b — FederalWorksheetVsTableThreshold scales with multiplier: income just above old threshold uses table at higher multiplier")]
    public void CalculateTaxLiabilityForYear_WorksheetThreshold_ScalesWithMultiplier()
    {
        // TaxConstants.FederalWorksheetVsTableThreshold = $100,000
    ...
    [Fact(DisplayName = "§16.11 — ProcessSocialSecurityCheck with multiplier=1.20 deposits 20% more cash than multiplier=1.0")]
    ...
    [Fact(DisplayName = "§16.12 — DeductPreTax with multiplier=1.5 deducts 50% more for health deductions")]
    ...
    [Fact(DisplayName = "§16.13 — AddPaycheckRelatedRetirementSavings with multiplier=1.10 invests 10% more in HSA")]
    public void AddPaycheckRelatedRetirementSavings_WithMultiplier_ScalesHsaContribution()
    {
        ...
        // Both amounts are rounded independently by the investment function (Math.Round(amount, 2) and
        // Math.Round(quantity, 4)), so exact equality depositAtOne × 1.1 == depositAtOnePointOne cannot
        // be relied on. Compare the ratio instead, accepting ±0.005 tolerance (precision=2).
        Assert.Equal(1.1m, depositAtOnePointOne / depositAtOne, 2);
    }
    ...
    [Fact(DisplayName = "§16.14 — AddPaycheckRelatedRetirementSavings with multiplier=1.10 invests 10% more in Roth 401k")]
    public void AddPaycheckRelatedRetirementSavings_WithMultiplier_ScalesRoth401KContribution()
    {
        ...
        // Both amounts are rounded independently by the investment function (Math.Round(amount, 2) and
        // Math.Round(quantity, 4)), so exact equality depositAtOne × 1.1 == depositAt1pt1 cannot
        // be relied on. Compare the ratio instead, accepting ±0.005 tolerance (precision=2).
        Assert.Equal(1.1m, depositAt1pt1 / depositAtOne, 2);
    }
    ...
    [Fact(DisplayName = "§16.15a — 401k contribution below IRS limit: full scaled amount is contributed")]
    public void AddPaycheckRelatedRetirementSavings_ContributionBelowIrsLimit_FullScaledAmountContributed()
    {
        // TaxConstants.Irs401KElectiveDeferralLimit = $23,000
        ...
        var model    = TestDataManager.CreateTestModel();
        model.RetirementDate = new LocalDateTime(2050, 1, 1, 0, 0);
        // Use price=1m so CurrentValue = Math.Round(dollarAmount, 2) exactly — avoids extra rounding
        // from the share/price conversion (quantity = Math.Round(amount/price, 4) * price).
        var prices   = TestDataManager.CreateTestCurrentPrices(0.07m, 1m, 1m, 1m);
    ...
    [Fact(DisplayName = "§16.15b — 401k contribution above IRS limit: capped at IRS limit × multiplier")]
    ...
    [Fact(DisplayName = "§16.16 — DeductPostTax with multiplier=1.25 deducts 25% more for insurance")]
    ...
    [Fact(DisplayName = "§16.17 — SetLongTermGrowthRateAndPrices with CpiGrowth=0.01 on multiplier=1.05 yields 1.0605")]
    ...
    [Fact(DisplayName = "§16.17b — CopyPrices preserves CumulativeCpiMultiplier")]
    ...
    [Fact(DisplayName = "§16.17c — CumulativeCpiMultiplier starts at 1.0 and initializes correctly")]
    ...
    [Fact(DisplayName = "§16.17d — SetLongTermGrowthRateAndPrices with negative CpiGrowth decreases multiplier (deflation scenario)")]
    ...
    [Fact(DisplayName = "§16.19 — CalculateIncomeRoom with multiplier=2.0 returns double the baseline headroom (empty ledger)")]

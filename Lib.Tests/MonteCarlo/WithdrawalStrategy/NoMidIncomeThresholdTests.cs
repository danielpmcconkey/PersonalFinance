...
public class NoMidIncomeThresholdTests
{
        ...
        int count, decimal amountPerPosition, decimal pricePerShare, McInvestmentAccountType accountType)
    {
        return TestDataManager.CreateTestInvestmentAccount(
            CreateInvestmentPositions(
                count, amountPerPosition, pricePerShare, McInvestmentPositionType.LONG_TERM)
            , accountType);
    }
        ...
        decimal expectedEndBalTaxFree)
    {
        ...
        accounts.InvestmentAccounts.Add(
            ...
            pricePerShare, McInvestmentAccountType.TRADITIONAL_401_K));
        accounts.InvestmentAccounts.Add(
            CreateInvestmentAccountWithPositions(countTaxable, amountPerPosition,
                pricePerShare, McInvestmentAccountType.TAXABLE_BROKERAGE));
        accounts.InvestmentAccounts.Add(
            CreateInvestmentAccountWithPositions(countTaxFree, amountPerPosition,
                pricePerShare, McInvestmentAccountType.ROTH_IRA));
    ...
    [Fact]
    public void RebalancePortfolio_YieldsCorrectCashAndLong()
    {
        // Arrange
    ...
    [Fact]
    public void Rebalance_MovesFromTaxDeferredAccountsFirst()
    {
        var model = TestDataManager.CreateTestModel(WithdrawalStrategyType.NoMidIncomeThreshold);
        model.RebalanceFrequency = RebalanceFrequency.MONTHLY;
        var person = TestDataManager.CreateTestPerson();
        ...
        // somewhere around $20k cash will be needed
        var cashNeededOnHand =
            Spend.CalculateCashNeedForNMonths(model, person, accounts, currentDate, model.NumMonthsCashOnHand);
        
        // determine starting amounts and expected results
        var tradShortfall = 2000m; // the amount of cash needed that will be beyond what the trad account can provide
        var startingTraditional = cashNeededOnHand - tradShortfall; // trad should be totally consumed
        var expectedTradMidBalance = 0m;
        var expectedTradLongBalance = 0m;
        var expectedIraDistributions = startingTraditional;
        
        var startingBrokerage = cashNeededOnHand * 2; // brokerage should move second
        var expectedBrokerageMidBalance = 0m;
        var expectedBrokerageLongBalance = startingBrokerage - tradShortfall;
        var expectedCapitalGains = tradShortfall * .5m; // half of what was sold because we set up our positions to have a cost basis of 50%
        
        var startingRoth = 2000m; // still under our total cash needed line; roth shouldn't get touched
        var expectedRothMidBalance = 0m;
        var expectedRothLongBalance = startingRoth;
        ...
        while (totalTraditionalSoFar < startingTraditional)
        {
        ...
        while (totalRothSoFar < startingRoth)
        {
            var position1 = TestDataManager.CreateTestInvestmentPosition(
                priceEach, quantityEach, McInvestmentPositionType.LONG_TERM);
            position1.InitialCost = priceEach * quantityEach;
        ...
        while (totalBrokerageSoFar < startingBrokerage)
        {
            var position1 = TestDataManager.CreateTestInvestmentPosition(
                priceEach, quantityEach, McInvestmentPositionType.LONG_TERM);
    ...
    [Fact]
    public void RebalancePortfolio_AtRightTimeAndWithEnoughCash_DoesNothing()
    {
        // Arrange
        ...
        var cashNeededTotal = Spend.CalculateCashNeedForNMonths(
            model, person, accounts, currentDate, model.NumMonthsCashOnHand);
        var cashToDeposit = cashNeededTotal * 1.25m;
        accounts = AccountCashManagement.DepositCash(accounts, cashToDeposit, currentDate).accounts;
        var midNeededTotal = 0m;
        
        
        var expectedCashBalance = cashToDeposit;
        var expectedMidBalance = 0m;
        var expectedLongBalance = longtermTotalWanted;

...
| `CalculateMonthlyHealthSpend` in Medicare band (ages 65–87): given known inputs (age, multiplier), the result equals the independently-computed expected value derived from the Medicare formula (Part A deductible × admissions, Part B premium + deductible, Part D premium + drug cost, each dollar constant scaled by `cumulativeCpiMultiplier`). Verified at ages 65 and 70, multipliers 1.0, 1.5, and 2.0 via `[Theory]`/`[MemberData]`. | A | Traces to FSD-003 / BR-1 (Q3). InflationAdjustmentTests: §16.3 — `CalculateMonthlyHealthSpend_MedicareBand_KnownInputsMatchExpected` |
| `CalculateMonthlyHealthSpend` pre-age-65 (Path A, retired) and age-88+ (Path B, assisted living): given known inputs (birth year, test year, multiplier), the result equals the exact expected value. Path A: `RequiredMonthlySpendHealthCare × multiplier` (800 × 1.0 = 800; 800 × 1.5 = 1200). Path B: `RequiredMonthlySpendHealthCare × 2m × multiplier` (800 × 2 × 1.0 = 1600; 800 × 2 × 2.0 = 3200). Verified via `[Theory]`/`[InlineData]`. | A | Traces to FSD-003 / BR-1. InflationAdjustmentTests: §16.3 — `CalculateMonthlyHealthSpend_PreMedicareAndAssistedLiving_KnownInputsMatchExpected` |

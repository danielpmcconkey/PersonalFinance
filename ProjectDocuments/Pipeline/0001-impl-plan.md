...
- **Risk (OI-4 — RESOLVED):** The original risk note referenced a test named `CalculateMonthlyHealthSpend_MedicareBand_MultiplierDoubles`. That test was never written. The test file already contains two `[Theory]`-based §16.3 tests — `CalculateMonthlyHealthSpend_PreMedicareAndAssistedLiving_KnownInputsMatchExpected` and `CalculateMonthlyHealthSpend_MedicareBand_KnownInputsMatchExpected` — both of which assert exact outputs given known inputs, with no directional-only assertions. The `MedicareBandExpected()` helper independently reimplements the Spend.cs formula: `numHospitalAdmissions` is age-derived and is NOT scaled by the multiplier (matching the implementation intent), while the dollar constants are multiplied by `cumulativeCpiMultiplier`. Dan's requirement for known-input/known-output tests via `[Theory]` and `[InlineData]`/`[MemberData]` is fully satisfied. Activate §16.3 tests without modification.
...
4. **§16.3 test precision:** `CalculateMonthlyHealthSpend_MedicareBand_MultiplierDoubles` asserts an exact doubling at multiplier=2.0. The hospital admission count formula `1.5 + (yearsOver65/10)` is age-based and does NOT scale — only the per-admission deductible scales. The exact-doubling assertion will fail. Should the test be changed to a directional assertion (result at 2.0× is greater than at 1.0×), or should the admission count formula also scale?
   Dan's decision: I see what you mean. This test is flawed by design. I believe there should be a test that asserts correct outputs given known inputs (via theories and inline data). Otherwise the test is meaningless. Of course it'll be more expensive if the multiplier is higher. That's not testing anything meaningful to me. It could be that there's already a separate test that tests as I describe here. Please have the test agent review this response and address this concern with an update to the tests and project test documents.
   **RESOLVED (test agent review):** The test `CalculateMonthlyHealthSpend_MedicareBand_MultiplierDoubles` was never written. The test file at `InflationAdjustmentTests.cs` lines 239–296 already contains exactly the design Dan requested: two `[Theory]` tests with `[InlineData]` and `[MemberData]` rows, each asserting exact known-input/known-output values. `CalculateMonthlyHealthSpend_MedicareBand_KnownInputsMatchExpected` (line 278) uses `MedicareBandExpected()` (line 193) as an independent reference implementation — it correctly does NOT double the hospital admission count when the multiplier doubles (because admissions are age-derived, not dollar-denominated), and it does correctly scale every dollar constant by `cumulativeCpiMultiplier`. `CalculateMonthlyHealthSpend_PreMedicareAndAssistedLiving_KnownInputsMatchExpected` (line 245) tests Paths A and B with hardcoded expected values (e.g., `800 × 1.5 = 1200`). No test file changes are required. No test document changes are required. This open item is closed.
